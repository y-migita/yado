...
- 多重起動防止:`daemon.pid`とソケットへのヘルスチェックで判定。CLIはデーモン不在なら`node:child_process`の`spawn(detached: true, stdio: "ignore")`で`bun src/daemon.ts run`を起動して切り離す。
...
- **ループバックの両ファミリー対応**:Guestは`127.0.0.1`ではなく`::1`だけで待ち受けることがある。実機では、Vite 8がhost未指定のとき[::1]のみにbindする場合があることを確認済み。上流への接続(HTTP fetch/WebSocket)とHTTPプローブは両ファミリーを試行し、どちらで応答したかをGuestごとにメモリ内キャッシュする。接続拒否時はもう一方へフォールバックする。台帳スキーマは変えない。
...
- 3秒間隔:`lsof -nP -iTCP -sTCP:LISTEN`をパースし、(pid, port)を列挙。既知のGuest、デーモン自身、1024未満のポートを除外。
...
- 通知は次の2つを両方行う。
  - Ownerのttyが実在すれば`/dev/<tty>`に`\nyado ▸ http://<name>.local/ (auto check-in)\n`を書き込む
  - `osascript -e 'display notification ...'`でデスクトップ通知を出す
...
1. デーモンの稼働を確認する。不在なら起動し、`/health`をポーリングして待つ。
...
- name省略時はcwdのGuest。
- Owner照合:呼び出し元のtty(`ps -o tty= -p $$`相当)と台帳のowner.ttyを比較。不一致かつ`--force`なしなら停止せず、exit 3で「このGuestは別のOwnerが起動しています。ユーザーに確認した上で --force を付けて再実行してください」という定型メッセージを出す(SKILL.mdがこのexit codeを扱う)。
- 停止:pgidがあれば`kill -TERM -<pgid>`、5秒待ってから残っていれば`kill -KILL`。kind: "auto"でpgid不明ならpidへ送る。完了後デーモンへ`DELETE`。

### `yado ls [--json]`

台帳をテーブルで表示する。列はNAME、URL、PORT、OWNER、KIND、UPTIME。`--json`は台帳をそのまま出す(エージェント用)。
...
- 純粋ロジック(名前の正規化と連番、PM/フレームワーク判定、lsof出力のパース、HostからGuestの解決)は`bun test`のユニットテストを書く。
